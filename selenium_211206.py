...
font_path = 'C:/Windows/Fonts/NanumGothic.ttf' # 글씨체 나눔바른고딕 보통으로 지정
...
for n in range(1, y + 2):
    ...
    for m in range(1, 19):
        ...
        if hide_article != 0:
            ...
            wordcloud = WordCloud(width=500, height=500, margin=0, background_color='white', font_path='C:/Windows/Fonts/NanumGothic.ttf').generate(details)
...
data.to_csv('C:/Users/!!사용자이름!!/Desktop/{0}_카테고리추가.csv'.format(user), index=False, header=['닉네임', '사는 동네', '판매 품목', '품목 카테고리', '판매 내용'], encoding="utf-8-sig")
