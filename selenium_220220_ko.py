...
driver = webdriver.Chrome(r"C:\\Users\\chromedriver.exe") # 실행 안될 시 새로운 chromedriver 버전으로 변경
driver.get(url)
time.sleep(3)
    ...
    for m in range(1, 19): 
        ...
        if hide_article != 0:
            ...
            wordcloud = WordCloud(width=500, height=500, margin=0, background_color='white', 
                                    font_path = 'C:/Windows/Fonts/NanumGothic.ttf').generate(word_result) # 글씨체 수정 바람
...
# pyplot.show()
pyplot.savefig('C:/Users/jang/Desktop/PBL/{0}_matplotlib.png'.format(user))
...
# pyplot.show()
pyplot.savefig('C:/Users/jang/Desktop/PBL/{0}_wordcloud.png'.format(user))
# -> show() 함수 혹은 savefig 함수 둘 중 하나만 사용해야 함

## data 리스트 엑셀 파일로 저장
data = pd.DataFrame(data)  # 데이터 프레임으로 전환
data.to_csv('C:/Users/jang/Desktop/PBL/{0}_카테고리추가.csv'.format(user), index=False,
